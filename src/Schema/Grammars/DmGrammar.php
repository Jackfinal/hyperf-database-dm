...
namespace Hyperf\Database\Dm\Schema\Grammars;
...
class DmGrammar extends Grammar
{
    ...
    {
        return 'BEGIN
            FOR c IN (SELECT table_name FROM all_tables) LOOP
    ...
    /**
     * Create the column definition for a identity type.
     *
     * @param  \Hyperf\Support\Fluent  $column
     * @return string
     */
    protected function typeIdentity(Fluent $column)
    {
        return "int identity({$column->start}, {$column->step})";
    }
    /**
    ...
    {
        return "SELECT table_name FROM all_tables where OWNER='{$this->schema_prefix}'";
    }


    public function compileColumnListing($table)
    {
        return "select COLUMN_NAME as column_key, COLUMN_NAME as column_name, DATA_TYPE as data_type , DATA_TYPE as column_type from dba_tab_cols where OWNER = '{$this->schema_prefix}' and TABLE_NAME = '{$table}' ";
    }
}